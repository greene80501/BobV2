...
class ThreadRecord:
    ...
    __slots__ = ("id", "name", "path", "model", "cwd", "created_at", "updated_at")
...
class StateDb:
    ...
    async def upsert_thread(
        ...
        await self._conn.execute(
            """
            INSERT INTO threads (id, name, path, model, cwd, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            """,
            (id, name, path, model, cwd, now, now),
        )
        await self._conn.commit()
    ...
    async def list_threads(
        ...
        if cwd_filter is not None:
            cursor = await self._conn.execute(
                "SELECT * FROM threads WHERE cwd = ? ORDER BY created_at DESC LIMIT ? OFFSET ?",
